...
@DisplayName("CatalogSvcRepositoryTest")
@ActiveProfiles("integration")
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class CatalogSvcRepositoryTest {
	
	@Autowired
	CatalogSvcRepository  repository;

	private static CatalogSvcEntity bean;

	@BeforeAll
	static void initAll() {
		bean = new CatalogSvcEntity(1L, "Object", "Object description");
	}
	...
	void testFindById() {
		CatalogSvcEntity b = repository.findById(bean.getId()); 
		if(b != null) {
	...
	void testFindAll() {
		List<CatalogSvcEntity> list = (List<CatalogSvcEntity>) repository.findAll(); 
